...
public class ImplementHotelService {
	...
	public FullDataInstance[] getHotelWithLocationAndRating(int numberStar, double lon, double lat,
			float radius, String lang, int limit, int offset) {
		String queryString = "select distinct ?place where" +
				...
				+ radius
				+ " KM) " +
				 " { ?place vtio:hasGeoPoint ?loc. ?place rdf:type <http://hust.se.vtio.owl#Hotel>. ?place vtio:hasRating ?star. ?star vtio:hasNumberStar \""+numberStar+"\"^^<http://www.w3.org/2001/XMLSchema#integer>. } where{  } } LIMIT " + limit + " OFFSET " + offset;
		return soapServiceProxy.getiComCoreService().getFullDataInstace(
				queryString, lang, false, VTIO_REPOSITORY_KEY);
	}
	
	public FullDataInstance[] getHotelOfDistrictWithRating(int numberStar,
			String districtUri, String lang, int limit, int offset) {
		String queryString = "select distinct ?place where" +
				"{ ?place rdf:type <http://hust.se.vtio.owl#Hotel>. " +
						" ?place vtio:hasRating ?star. ?star vtio:hasNumberStar \""+numberStar+"\"^^<http://www.w3.org/2001/XMLSchema#integer>." +
	...
	public SimpleDataObject[] getAllAccomodationCategory(String lang) {
		String[][] cityStrings = soapServiceProxy
				.getiComCoreService()
				.executeQuery(
						"select ?uri ?label { ?uri rdf:type owl:Class.  ?uri rdfs:subClassOf vtio:Accomodation. { select ?label { ?uri rdfs:label ?label. FILTER(lang(?label)='"
								+ lang + "') } LIMIT 1 } }", true,
						VTIO_REPOSITORY_KEY);
		SimpleDataObject[] simpleDataObjects = new SimpleDataObject[cityStrings.length + 1];
		for (int i = 0; i < simpleDataObjects.length - 1; i++) {
			SimpleDataObject simpleDataObject = new SimpleDataObject();
			simpleDataObject.setUriString(cityStrings[i][0]);
			simpleDataObject.setLabelString(cityStrings[i][1]);
			simpleDataObjects[i] = simpleDataObject;
		...
		SimpleDataObject simpleDataObject = new SimpleDataObject();
		simpleDataObject.setUriString("http://hust.se.vtio.owl#Accomodation");
		simpleDataObject.setLabelString("Tất cả");
		simpleDataObjects[simpleDataObjects.length - 1] = simpleDataObject;
		return simpleDataObjects;
	}
	...
	public FullDataInstance[] getAllHotelOfDistrict(String categoryUri,
			String districtUri, String lang, int limit, int offset) {
		if (limit > 10)
			return null;
